...
// UC005: Relatar problema com pedido
export async function relatarProblema(id: number, problema: string): Promise<Pedido> {
  try {
    ...
    return await apiFetch(`/pedidos/${id}/relatar_problema/`, {
      method: "PATCH",
      body: JSON.stringify({ problema: problema.trim() }),
    });
  } catch (error) {
    console.error(`Erro ao relatar problema do pedido ${id}:`, error);
    throw error;
  }
}
